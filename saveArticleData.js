...
const filePath = path.join(__dirname, 'data', 'article_data.json');
...
async function main() {
    try {
        ...
        // Update list query
        const readQuery = fs.readFileSync('data/updateQuery.sql', 'utf-8');
        await pool.request().query(readQuery);

        // Fetch data from the database
        const result = await pool.request().query(`SELECT NAME, DESCRIPT_1 FROM [iX_1033].[dbo].[articles] WHERE DESCRIPT_2 = 'RIPO catalog' AND NAME NOT LIKE 'r_%'`);
